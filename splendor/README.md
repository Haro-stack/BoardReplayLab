# BoardReplayLab Splendor Module

Local replay capture and format conversion tools for Splendor experiments.
This module lives under `splendor/` because BoardReplayLab is intended to host
multiple board-game replay pipelines rather than only one Splendor workflow.
...
```bash
node splendor/scripts/bga-splendor-replay-crawler.mjs --table 854928957 --manual --wait-ms 300000
```

Convert the raw capture into Gem Table replay JSON:

```bash
node splendor/scripts/convert-splendor-capture.mjs --in bga-replays/bga-table-854928957-replay.json
```
...
```bash
node splendor/scripts/bga-splendor-replay-crawler.mjs --table 854928957 --manual
```
...
```bash
node splendor/scripts/convert-dinoboard-splendor.mjs \
